...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        $sql = 'UPDATE PickupEvents SET  EventName="' . $EventName . '", Sport="' . $Sport . '", Location="' . $Location . '", EventDateTime="' . $EventDateTime . '", Latitude="' . $Latitude . '", Longitude="' . $Longitude . '", AgeMax="' . $AgeMax . '", AgeMin="' . $AgeMin . '", MinUserRating="' . $MinUserRating . '", PlayerNumber="' . $PlayerNumber . '", IsPrivate="' . $IsPrivate . '", Gender="' . $Gender . '" 
